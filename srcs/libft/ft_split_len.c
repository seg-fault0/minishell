...
/*   Updated: 2025/05/16 19:22:25 by zogrir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *ft_create_word(const char *start, size_t len)
{
    ...
    if (len == 0)
        return (NULL);  // Don't allocate for empty words
    word = malloc(len + 1);
    if (!word)
        return (NULL);
    i = 0;
...
static char *ft_get_next_word(const char *s, char c, size_t *offset, size_t size)
{
    const char *start;
    size_t word_len;
